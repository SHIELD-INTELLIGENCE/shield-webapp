...
export default function Feeds() {
        ...
        try {
          ...
          unsubFirestore = onSnapshot(
            ...
            (err) => {
              ...
              setError(
                "Failed to load your feeds. Please refresh the page or try again later.",
              );
              setLoading(false);
            },
          );
  ...
  return (
    <div className="feeds-page">
      <h2 className="feeds-title">Your Feeds</h2>

      {loading ? (
        <div className="feeds-loading">

          <div className="feeds-loading-text">

            Loading Feeds...
          </div>
          <div className="shield-spinner feeds-spinner"></div>
        </div>
      ) : error ? (
            ...
            <div key={feed.id} className="feed-card">
              ...
              {(() => {
                ...
                if (["new", "pending"].includes(s))
                  statusClass += " feed-status--pending";
                else if (["in-progress", "progress", "working"].includes(s))
                  statusClass += " feed-status--in-progress";
                else if (["done", "completed", "resolved"].includes(s))
                  statusClass += " feed-status--done";
                else if (["failed", "error", "blocked"].includes(s))
                  statusClass += " feed-status--failed";
